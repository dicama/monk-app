...
class StackedChartElement extends BasicElement {
  final String key = "StackedChartSpark";

  Widget parseScript(String input, Module parent, String moduleId,
      {Map parentOptions}) {
    return StackedChartElementWidget(
...
class _StackedChartElementState extends State<StackedChartElementWidget> {
  var groupVal = -1;
  var func;
  ...
  void parseScriptMeta(String input) {
    final String datasrc = widget.options["datasrc"]; // read the params
    ...
    var stackedAlldays = Map<int, Map<int, DataPoint>>();
    var stackedAlldaysCount = Map<int, Map<int, int>>();

    array.forEach((element) {
      if (!stackedAlldays.keys.contains(element[properties[1]])) {
        stackedAlldays[element[properties[1]]] = Map<int, DataPoint>();
        stackedAlldaysCount[element[properties[1]]] = Map<int, int>();
      }
    });
    ...
    if (domain == "day") {
      ...
      while (newDate.isBefore(now)) {
        stackedAlldays.forEach((key, value) {
          stackedAlldays[key][newDate.millisecondsSinceEpoch] =
              DataPoint(newDate, 0, Colors.transparent);
          stackedAlldaysCount[key][newDate.millisecondsSinceEpoch] = 0;
        });
        yo.add(charts.TickSpec(newDate.millisecondsSinceEpoch.toString(),
            label: getLabel(newDate)));
    ...
    array.forEach((element) {
      ...
      if (date1.isAfter(start) && date1.isBefore(now)) {
        ...
        if (stackedAlldays[element[properties[1]]]
            .containsKey(date1.millisecondsSinceEpoch)) {
          val += stackedAlldays[element[properties[1]]]
                  [date1.millisecondsSinceEpoch]
              .number;

          stackedAlldaysCount[element[properties[1]]]
                  [date1.millisecondsSinceEpoch] =
              stackedAlldaysCount[element[properties[1]]]
                      [date1.millisecondsSinceEpoch] +
                  1;
        }
        stackedAlldays[element[properties[1]]][date1.millisecondsSinceEpoch] =
            new DataPoint(date1, val, Color(element[properties[1]]));
      }
    });

    List<charts.Series<dynamic, String>> stackedSeries = [];
    stackedAlldays.forEach((key, value) {
      stackedSeries.add(charts.Series<DataPoint, String>(
        id: 'Poop+$key',
        domainFn: (DataPoint clickData, _) =>
            clickData.date.millisecondsSinceEpoch.toString(),
        measureFn: (DataPoint clickData, _) => clickData.number,
        colorFn: (DataPoint clickData, _) => clickData.color,
        fillColorFn: (DataPoint clickData, _) => clickData.color,
...
class StackedChartElementWidget extends StatefulWidget {
  final String input;
  final String moduleId;
  final Map options;

  StackedChartElementWidget(this.input, this.moduleId, this.options);

  @override
  _StackedChartElementState createState() => _StackedChartElementState();
}
